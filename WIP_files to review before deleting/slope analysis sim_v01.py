...
from accim.parametric_and_optimisation.objectives import return_time_series, sum_results
...
[i for i in df_output_variables_idf.variable_name if 'Occupied Comfortable' in i or 'Occupied Discomfortable' in i]
...
df_output_variables_idf = df_output_variables_idf[
    (
        ...
        |
        df_output_variables_idf['variable_name'].str.contains('AFN Zone Ventilation Air Change Rate_Building_Average')
        |
        df_output_variables_idf['variable_name'].str.contains('Occupied Comfortable Hours_No Applicability_Building_Average')
        |
        df_output_variables_idf['variable_name'].str.contains('Occupied Discomfortable Hours_No Applicability_Building_Average')
...
df_output_meters_testsim, df_output_variables_testsim = parametric.get_outputs_df_from_testsim(reduce_sim_time=True)
df_output_meters_testsim
df_output_variables_testsim

# rdd = get_rdd_file_as_df()
# mdd = get_mdd_file_as_df()
# mtd = parse_mtd_file()
...
df_output_variables_testsim_input = df_output_variables_testsim.copy()

df_output_variables_testsim_input['func'] = return_time_series

for i in df_output_variables_testsim_input.index:
    if 'Hours' in df_output_variables_testsim_input.loc[i, 'variable_name']:
        df_output_variables_testsim_input.loc[i, 'func'] = sum_results
...
parametric.set_outputs_for_simulation(
    ...
    df_output_variable=df_output_variables_testsim_input,
)
...
parametric.run_parametric_simulation(
    ...
    out_dir='sim_runs_v01',
    df=all_combinations[4:6],
...
parametric.outputs_param_simulation.to_excel('param_sim_outputs_v01.xlsx')

parametric.get_hourly_df()
parametric.outputs_param_simulation_hourly.to_excel('param_sim_outputs_hourly_v01.xlsx')