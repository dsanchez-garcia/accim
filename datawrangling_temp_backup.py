class Table:
    ...
    def __init__(self,
        ...
        if 'hourly' in self.frequency or 'timestep' in self.frequency:
            self.df['Hour'] = (pd.to_numeric(self.df['Hour']) - 1).astype(str).str.pad(width=2, side='left',
                                                                                       fillchar='0')
            ...
            for j in range(len(self.cols)):
                if len(adap_vs_stat_data_y_main) > 0:
                    temp = [
                        ...
                        [
                            self.df_for_graph[[x for x in self.df_for_graph.columns if
                                               self.rows[i] in x and baseline in x and dataset in x]]
                    ...
                    temp = [
                        ...
                        f'{self.rows[i]}_{self.cols[j]}',
                        self.df_for_graph[[x for x in self.df_for_graph.columns if
                                           self.rows[i] in x and self.cols[j] in x and data_on_x_axis in x]]
                    ]
    ...
    def time_plot(
        ...
        if confirm_graph:
            ...
            start_date = datetime.datetime.strptime(self.df_for_graph['Date/Time'][0],
                                                    freq_graph_dict[self.frequency][1])
