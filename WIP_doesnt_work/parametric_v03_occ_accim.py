...
building = ef.get_building(
    ...
    # 'TestModel_onlyGeometryForVRFsystem_2zones_CalcVent_V940.idf'
...
# df_apmv_arguments = apmv.generate_df_from_args(building=building)
...
# besos.eplus_funcs.run_building(building)
...
def change_adap_coeffs(building, value):
    bf.change_adaptive_coeff_all_zones(building=building, value=value)

def change_sps(building, value):
    bf.change_pmv_setpoint_all_zones(building=building, value=value)
...
parameters = [
    Parameter(
        ...
        selector=GenericSelector(set=change_adap_coeffs),
        value_descriptor=RangeParameter(min_val=-2, max_val=2)
    ),
    Parameter(
        name='PMV',
        selector=GenericSelector(set=change_sps),
