...
def _find_branch_for_object(idf, obj_name: str) -> Optional[str]:
    ...
    for branch in _idfobjects_safe(idf, 'Branch'):
        for fname in _field_names(branch):
            if fname.startswith('Component_') and fname.endswith('_Name'):
...
def _find_branchlist_for_branch(idf, branch_name: str) -> Optional[str]:
    ...
    for bl in _idfobjects_safe(idf, 'BranchList'):
        for fname in _field_names(bl):
            if fname.startswith('Branch_') and fname.endswith('_Name'):
...
def _find_supply_outlet_node(idf, airloop_name: str) -> List[str]:
    """Return the demand-side inlet node(s) of an AirLoopHVAC.
    These are the nodes that connect to the SupplyPath."""
    nodes: List[str] = []
    for loop in _idfobjects_safe(idf, 'AirLoopHVAC'):
        if loop.Name == airloop_name:
            node_or_list = _get_field(loop, 'Demand_Side_Inlet_Node_Names')
            if not node_or_list:
                node_or_list = _get_field(loop, 'Demand_Side_Inlet_Node_Name')
            
            is_nodelist = False
            for nl in _idfobjects_safe(idf, 'NodeList'):
                if nl.Name == node_or_list:
                    is_nodelist = True
                    for fname in _field_names(nl):
                        if fname.startswith('Node_') and fname.endswith('_Name'):
                            n = _get_field(nl, fname)
                            if n and n not in nodes:
                                nodes.append(n)
            
            if not is_nodelist and node_or_list:
                nodes.append(node_or_list)
    return nodes


def _find_zone_splitter_outlets(idf, supply_outlet_nodes: List[str]) -> List[str]:
    """
                        ...
                        for splitter in _idfobjects_safe(idf, 'AirLoopHVAC:ZoneSplitter'):
                            if splitter.Name == splitter_name:
                                for sf in _field_names(splitter):
                                    if sf.startswith('Outlet_') and sf.endswith('_Node_Name'):
...
def _zones_from_splitter_outlets(idf, outlet_nodes: List[str]) -> List[str]:
    ...
    If an AirTerminal object is placed between the splitter and the zone,
    this function traces the node through the AirTerminal.
    """
    zones: List[str] = []
    
    # Expand outlet_nodes by tracing through AirTerminal:* objects
    # OpenStudio often places an AirTerminal between the splitter and the zone connection.
    extended_outlets = list(outlet_nodes)
    for obj_type in idf.idfobjects:
        if obj_type.startswith('AIRTERMINAL:'):
            for term in _idfobjects_safe(idf, obj_type):
                inlet = None
                outlet = None
                for fname in _field_names(term):
                    if 'Inlet' in fname and 'Node' in fname:
                        inlet = _get_field(term, fname)
                    if 'Outlet' in fname and 'Node' in fname:
                        outlet = _get_field(term, fname)
                if inlet and inlet in extended_outlets and outlet and outlet not in extended_outlets:
                    extended_outlets.append(outlet)
    ...
    for conn in _idfobjects_safe(idf, 'ZoneHVAC:EquipmentConnections'):
        ...
        for nl in _idfobjects_safe(idf, 'NodeList'):
            if nl.Name == inlet:
                for fname in _field_names(nl):
                    if fname.startswith('Node_') and fname.endswith('_Name'):
                        if _get_field(nl, fname) in extended_outlets:
...
def _strategy_c2(idf, obj_name: str) -> Optional[List[str]]:
    ...
    supply_outlets = _find_supply_outlet_node(idf, airloop_name)
    if not supply_outlets:
        return None

    splitter_outlets = _find_zone_splitter_outlets(idf, supply_outlets)
    if not splitter_outlets:
        # Some loops have no splitter (single-zone AHU); the outlet node
        # connects directly to the zone inlet
        zones = _zones_from_splitter_outlets(idf, supply_outlets)
    else:
