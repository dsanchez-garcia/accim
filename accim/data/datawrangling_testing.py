                ...
                for i in range(len(rows)):
                    ...
                    for j in range(len(cols)):
                        temp = [
                            [i, j],
                            f'{rows[i]}_{cols[j]}',
                            [i for i in data_on_x_axis],
                            [self.df_for_graph[
                                 (self.df_for_graph['col_to_gather_in_rows'] == rows[i]) &
                                 (self.df_for_graph['col_to_gather_in_cols'] == baseline)
                                 ][k]
                             for k in data_on_x_axis
                             ],
                            # [i for i in colorlist]
            ...
            for i in range(len(rows)):
                for j in range(len(cols)):
                    ...
                    for k in range(len(self.y_list_main_scatter[i][j][3])):
                        for m in range(len(self.x_list[i][j][3])):
                            if self.y_list_main_scatter[i][j][2][k] in self.x_list[i][j][2][k]:
                                if i == 0 and j == 0:
                                    ax[i, j].scatter(
                                        self.x_list[i][j][3][m],
