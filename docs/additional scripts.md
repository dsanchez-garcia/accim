...
>>> run.runEp94()
```
`runEp94()` is going to ask you if you want to run the simulations only with IDF files generated by ACCIM. If you enter 'y', it'll run only ACCIM output IDFs, otherwise if you enter 'n', it will run all idfs in the folder. As you can see below, we didn't need to remove the original IDF 'TestModel_SingleZone.idf' from the folder. Then, it will let you know the IDFs and EPWs that are going to be used in the simulations. Besides, it'll let you know the total number of simulations, and will ask for your confirmation, because you might start thousands of simulations by mistake.
```
>>> run.runEp94()
