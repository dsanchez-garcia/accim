...
First you need to import the class `AddAccis` (ACCIS stands for Adaptive-Comfort-Control-Implementation Script):

```python
>>> from accim.sim import AddAccis
```

And then, you just need to instantiate it:
...
```
--------------------------------------------------------
Adaptive-Comfort-Control-Implemented Model (ACCIM) v1.0.0
--------------------------------------------------------
...
-    Using run_ep() to directly run simulations with EnergyPlus
...
Enter the EnergyPlus version (9.1 to 25.1, or auto): 23.1
...
*   script_type can be ‘vrf_mm’, ‘vrf_ac’, ‘ex_mm’ or ‘ex_ac’, and it refers to the type of functions as explained above
*   supply_air_temp_method can be ‘supply air temperature’ or ‘temperature difference’, and it is the supply air temperature input method for the VRF systems.
...
If you have run `AddAccis()`, you will be asked in the prompt to enter a few more values separated by space to set up the desired IDFs. However, you can also skip the command prompt process by instantiating `AddAccis` directly including the arguments, whose usage would be:

```python
>>> AddAccis(script_type=str, # 'vrf_mm', 'vrf_ac', 'ex_mm', 'ex_ac'
>>>          supply_air_temp_method=str, # 'supply air temperature', 'temperature difference'
>>>          output_keep_existing=bool, # True or False
>>>          output_type=str, # 'simplified', 'standard', 'detailed' or 'custom'
>>>          output_freqs=list, # ['timestep', 'hourly', 'daily', 'monthly', 'runperiod']
>>>          output_gen_dataframe=bool, # True or False
>>>          output_take_dataframe=..., # a pandas DataFrame
>>>          energyplus_version=str, # '9.1' to '25.1', or 'auto'
>>>          temp_control=str, # 'temperature' or 'temp', or 'pmv'
>>>          comfort_standard=list, # the Comfort Standard
>>>          category=list, # the Category
>>>          comfort_mode=list, # the Comfort Mode
>>>          setpoint_accuracy=float, # defines the accuracy of the setpoint temperatures
>>>          cooling_season_start=..., # str date dd/mm, or an int day of the year
>>>          cooling_season_end=..., # str date dd/mm, or an int day of the year
>>>          hvac_mode=list, # the HVAC mode
>>>          vent_control=list, # the Ventilation Control
>>>          vof_max_temp_diff=float,
>>>          vof_min_temp_diff=float,
>>>          vof_multiplier=float,
>>>          vent_setpoint_offset=list,
>>>          min_outdoor_temp_offset=list,
>>>          max_wind_speed=list,
>>>          ast_tol_start=float, # start of the tolerance sequence
>>>          ast_tol_end=float, # end of the tolerance sequence
>>>          ast_tol_steps=float, # steps of the tolerance sequence
>>>          name_suffix=str, # some text to add at the end of the output IDF file name
>>>          verbose=bool, # True to print all process in screen. Default is True.
>>>          confirm_generation=bool, # True to auto-confirm IDF generation; if False, you'll be asked in the prompt. Default is False.
>>>          )
```
...
```python
>>> AddAccis(script_type='vrf_mm', # 'vrf_mm', 'vrf_ac', 'ex_mm', 'ex_ac'
>>>          supply_air_temp_method='supply air temperature', # 'supply air temperature', 'temperature difference'
>>>          output_keep_existing=False, # True or False
>>>          output_type='standard', # 'simplified' or 'standard'
>>>          output_freqs=['hourly', 'runperiod'], # ['timestep', 'hourly', 'daily', 'monthly', 'runperiod']
>>>          output_gen_dataframe=False,
>>>          # we just omit output_take_dataframe
>>>          energyplus_version='9.5', # '9.1' to '25.1', or 'auto'
>>>          temp_control='temp', # Temperature Control: 'temperature' or 'temp', or 'pmv'
>>>          comfort_standard=[0, 1, 2, 3], # the Comfort Standard
>>>          category=[1, 2, 3, 80, 90], # the Category
>>>          comfort_mode=[0, 1, 2, 3], # the Comfort Mode
>>>          setpoint_accuracy=10, # Therefore, setpoints will be rounded to the first decimal
>>>          # we just omit cooling_season_start, since the default date is May 1st
>>>          # we just omit cooling_season_end, since the default date is September 1st
>>>          hvac_mode=[0, 1, 2], # the HVAC mode
>>>          vent_control=[0, 1], # the Ventilation Control
>>>          vof_max_temp_diff=20, # When the difference of operative and outdoor temperature exceeds 20°C, windows will be opened the fraction of vof_multiplier.
>>>          vof_min_temp_diff=1, # When the difference of operative and outdoor temperature is smaller than 1°C, windows will be fully opened. Between min and max, windows will be linearly opened.
>>>          vof_multiplier=0.2, # Fraction of window to be opened when temperature difference exceeds vof_max_temp_diff.
>>>          vent_setpoint_offset=[0, 1, 2], # the Ventilation Setpoint Temperature offset
>>>          min_outdoor_temp_offset=[0, 1, 2], # the Minimum Outdoor Temperature offset
>>>          max_wind_speed=[10, 20, 30], # the Maximum Wind Speed
>>>          ast_tol_start=0, # the start of the tolerance sequence
>>>          ast_tol_end=2, # the end of the tolerance sequence
>>>          ast_tol_steps=0.25, # the steps of the tolerance sequence
>>>          name_suffix='standard' # for example, just in case you want to clarify the outputs
>>>          )
```

For clarity purposes, it’s recommended to specify the argument name as well, as shown above. If you don’t specify all arguments, you’ll be ask to enter them at the prompt command, and these values will be used instead of those specified in the function call. Each argument is explained below:

-   comfort_standard: refers to the thermal comfort standard or model to be applied. Enter any number from 0 to 22 or 99 to select the comfort standard or model to be used; you can see which model is each number in the table below. For example, if you enter ‘0 1 2 3’, you’ll get IDFs for CTE, EN16798-1, ASHRAE 55 and the local model developed by Rijal et al for Japanese dwellings. If you enter 99, that means you want to use some custom adaptive model. Therefore, you'll be asked to enter the slope, y-intercept among other parameters. If you don’t enter any number, it’ll ask you to enter the numbers again.
...
**TestModel_onlyGeometryForVRFsystem[CS_INT EN16798[CA_1[CM_3[HM_2[VC_0[VO_0.0[MT_50.0[MW_50.0[AT_0.1[standard**
...
(If it is too small, you can look at it also at the [Github repository](https://htmlpreview.github.io/?https://github.com/dsanchez-garcia/accim/blob/master/accim/docs/html_files/full_setpoint_table.html))
...
You can also execute it at your computer. You just need to find the folder containing the .ipynb and all other files at the accim package folder within your site_packages path, in
...
You can also see an example below. The input file is included within `accim/sample_files/sample IDFs` folder, and it was originally named `TestModel_onlyGeometryForVRFsystem_2zones_CalcVent_V2310.idf`, but for clarity purposes in this case has been renamed to “TestModel.idf”.
...
>>> dir(run)
['IDF', '__builtins__', '__cached__', '__doc__', '__file__', '__loader__', '__name__', '__package__', '__spec__', 'make_eplaunch_options', 'os', 'removefiles', 'run_ep', 'run_idfs']
>>> run.run_ep()
```

`run_ep()` is going to ask you if you want to run the simulations only with IDF files generated by accim. If you enter ‘y’, it’ll run only accim output IDFs, otherwise if you enter ‘n’, it will run all idfs in the folder. As you can see below, we didn’t need to remove the original IDF ‘TestModel_SingleZone.idf’ from the folder. Then, it will let you know the IDFs and EPWs that are going to be used in the simulations. Besides, it’ll let you know the total number of simulations, and will ask for your confirmation, because you might start thousands of simulations by mistake. Further, `run_ep()` can take a total number of 3 arguments, which are 2 boolean arguments, and one integer: `run_ep(run_only_accim=True, confirm_run=True, num_cpus=4)`. If you entered these, you would skip the command prompt process and jump straight to the simulation process. Since we entered 4 for the num_cpus argument, the simulations would be run by using 4 CPS at the same time.
