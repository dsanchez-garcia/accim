...
- **EPW/IDF Category Name Collision in `apply_category_mapping`**: Fixed a silent data-loss bug where defining a category with the same name (e.g. `'type'`) for both EPW and IDF mapping rules caused the IDF values to overwrite the EPW values. `apply_category_mapping` now detects collisions between EPW and IDF category names before applying them and automatically renames conflicting EPW categories with an `'epw_'` prefix (e.g. `'type'` → `'epw_type'`), emitting a `UserWarning` that instructs the user to update their `highlight_dict`, `col`, `row`, or `hue` arguments accordingly. This restores the expected behaviour of `highlight_dict` when EPW and IDF share a category name.
- **Boxplot Highlight Legend Placement**: Fixed the highlight legend being placed inside the first subplot axes, overlapping data. Highlight handles are now merged into the FacetGrid's figure-level `hue` legend on the right side of the figure, with a bold `― Highlights ―` section divider separating the hue (performance) entries from the highlight entries.
- **Boxplot Subplot Size Customisation**: Added `height` (default `4`), `aspect` (default `1.2`), and `figsize` parameters to `plot_categorical_boxplots()`. `height` and `aspect` are forwarded directly to `sns.catplot`; `figsize` (a `(width, height)` tuple) is applied after the FacetGrid is built and always takes precedence. The returned `FacetGrid` object can be further customised post-call via `g.fig.set_size_inches(...)`, `g.set_titles(...)`, etc.
- **EPW Suffix Category Persistence (`add_epw_suffix_category`)**: Added `SimulationBase.add_epw_suffix_category(col_name, suffix_map, fallback)` to derive a new category column from the last `'_'`-delimited suffix of each EPW filename (e.g. `'tmy'`, `'met'`, `'2015'`) and persist the derivation rule inside `DataFrame.attrs`. Both `load_outputs_parametric` and `load_outputs_optimisation` now automatically restore and re-apply any stored suffix rules after loading from a pickle, so the column is always present without requiring any manual post-load calls.
- **Session Merge (`merge`)**: Added `SimulationBase.merge(other, inplace=False)` to combine two simulation instances by concatenating all result DataFrames (`outputs_param_simulation`, `*_hourly`, `*_monthly`, `outputs_optimisation`, etc.). Metadata (`building_floor_area`, `epws`, `epw_mapping_rules`, `idf_mapping_rules`, `epw_suffix_categories`) is merged with `self` taking priority; conflicting scalar values emit a `UserWarning`. By default a new instance is returned; pass `inplace=True` to modify `self` directly.
