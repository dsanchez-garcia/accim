...
temp_plot = sns.FacetGrid(
    ...
    row_order=['Asahikawa', 'Sapporo', 'Morioka', 'Niigata', 'Maebashi', 'Tokyo', 'Kagoshima', 'Naha'],
    hue='EPW_Year',
    sharex=True,
    sharey='row',
    margin_titles=True,
...
##
vars_to_analyse_table = [
    'Site Outdoor Air Drybulb Temperature (°C)',
    # 'Site Outdoor Air Relative Humidity (%)'
]

site_table_dict = {}
for i in vars_to_analyse_table:
    temp = {i: 'mean'}
    site_table_dict.update(temp)
...
df_for_table_monthly_site = z.df.copy()
# z.df = z.df.groupby(['Source', 'Month', 'Day']).agg(site_dict)
df_for_table_monthly_site.columns
df_for_table_monthly_site = df_for_table_monthly_site.drop('Site Outdoor Air Relative Humidity (%)', axis=1)

df_for_table_monthly_site = df_for_table_monthly_site.groupby(['Source', 'Month']).agg(site_table_dict)

df_for_table_monthly_site = df_for_table_monthly_site.unstack('Month')

##
df_for_table_monthly_site.columns = df_for_table_monthly_site.columns.map('_'.join).str.strip('_')

# df_for_table_monthly_site['new'] = ['_'.join(map(str, i)) for i in df_for_table_monthly_site.index.tolist()]

df_for_table_monthly_site.reset_index(inplace=True)

df_for_table_monthly_site['data'] = 'temp'

# df_for_table_monthly_site = df_for_table_monthly_site.set_index([pd.RangeIndex(len(df_for_table_monthly_site))])

for i in range(len(df_for_table_monthly_site)):
    df_for_table_monthly_site.loc[i, 'data'] = str(df_for_table_monthly_site.loc[i, 'Source']).split('[')[-1]


df_for_table_monthly_site[[
    'EPW_Country_name',
    'EPW_City_or_subcountry',
    'EPW_Scenario-Year',
    # 'Month',
    ]] = df_for_table_monthly_site['data'].str.split('_', expand=True)

df_for_table_monthly_site = df_for_table_monthly_site.drop('Source', axis=1)
df_for_table_monthly_site = df_for_table_monthly_site.drop('data', axis=1)


# df_for_table_monthly_site['EPW_Country_City_Scenario-Year'] = df_for_table_monthly_site[[
#     'EPW_Country_name',
#     'EPW_City_or_subcountry',
#     'EPW_Scenario-Year',
#     ]].agg('_'.join, axis=1)
...
#     'EPW_Scenario',
#     'EPW_Year',
# ]] = df_for_table_monthly_site['EPW_Scenario-Year'].str.split('-', expand=True)
# df_for_table_monthly_site.EPW_Year.fillna(value='Present', inplace=True)

##
df_for_table_monthly_site = df_for_table_monthly_site.drop('EPW_Country_name', axis=1)

df_for_table_monthly_site.set_index(['EPW_City_or_subcountry', 'EPW_Scenario-Year'], inplace=True)

df_for_table_monthly_site.to_excel('outdoor_mean_temp.xlsx')